...
import { AlertTriangle, CheckCircle2, CopyCheck, HelpCircle } from "lucide-react";
      ...
      {imports.length > 0 ? (
        <>
          ...
          <div className="mt-4 hidden overflow-hidden rounded-lg border border-slate-200 lg:block">
            <div className="grid grid-cols-[118px_150px_118px_96px_minmax(180px,1fr)_140px_72px] gap-3 bg-slate-50 px-3 py-2 text-xs font-black text-slate-500">
              <div>created_at</div>
            ...
            <div className="divide-y divide-slate-100">
              {imports.map((record) => (
                <ImportTableRow key={record.id} record={record} />
              ))}
            </div>
          </div>
        </>
...
function ImportCard({ record }: { record: GoogleFormImportRecord }) {
  return (
...
function ImportTableRow({ record }: { record: GoogleFormImportRecord }) {
  return (
    <div className={record.status === "error" || record.status === "pending_review" ? "grid grid-cols-[118px_150px_118px_96px_minmax(180px,1fr)_140px_72px] gap-3 bg-red-50/80 px-3 py-2.5 text-sm" : "grid grid-cols-[118px_150px_118px_96px_minmax(180px,1fr)_140px_72px] gap-3 px-3 py-2.5 text-sm"}>
